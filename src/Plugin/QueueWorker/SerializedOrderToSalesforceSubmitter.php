...
namespace Drupal\ilr_outreach_order_submitter\Plugin\QueueWorker;
...
/**
 * A Queue worker that submits serialized orders to a Salesforce webhook.
 *
 * Usage:
 * - Serialize an order to the https://github.com/ilrWebServices/ilr-salesforce-bridge-api specification.
 * - Add that data to this queue via \Drupal::service('queue')->get('serialized_order_to_salesforce')->createItem($serialized_order).
 *
 * @QueueWorker(
 *   id = "serialized_order_to_salesforce",
 *   title = @Translation("Serialized Order to Salesforce Submitter"),
 *   cron = {"time" = 120}
 * )
 */
class SerializedOrderToSalesforceSubmitter extends QueueWorkerBase implements ContainerFactoryPluginInterface {
